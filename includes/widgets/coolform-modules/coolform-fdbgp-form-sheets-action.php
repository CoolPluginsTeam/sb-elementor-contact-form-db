...
add_action( 'elementor/controls/register', function ( $controls_manager ) {
    ...
    class CoolForm_FDBGP_Control_Dynamic_Select2 extends \Elementor\Base_Data_Control {
    ...
    $controls_manager->register( new CoolForm_FDBGP_Control_Dynamic_Select2() );
} );

/**
 * Shared implementation for Cool Form / Cool FormKit Pro Google Sheets action.
 * Used via trait so we can extend either Action_Base depending on which plugin is active.
 */
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
trait CoolForm_FDBGP_Form_Sheets_Action_Trait {
    ...
    public function render_editor_script() {
        wp_enqueue_script(
            'fdbgp-editor-script',
            FDBGP_PLUGIN_URL . 'assets/js/fdbgp-editor.min.js',
...
if ( class_exists( 'Cool_FormKit_Pro\Modules\Forms\Classes\Action_Base' ) ) {
    // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
    class CoolForm_FDBGP_Form_Sheets_Action extends \Cool_FormKit_Pro\Modules\Forms\Classes\Action_Base {
        use CoolForm_FDBGP_Form_Sheets_Action_Trait;
    }
} elseif ( class_exists( 'Cool_FormKit\Modules\Forms\Classes\Action_Base' ) ) {
    // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
    class CoolForm_FDBGP_Form_Sheets_Action extends \Cool_FormKit\Modules\Forms\Classes\Action_Base {
        use CoolForm_FDBGP_Form_Sheets_Action_Trait;
    }
}
