...
class CFKEF_Save_Entries {
    ...
    public function __construct() {
        add_action('cfkef/form/entries', [ $this, 'save_entries' ], 10, 4);
    }

    public function save_entries($record, $ajax_handler, $collect_entries,$form_instance = 'cool-form') {
        if($form_instance === 'ehp-form'){
            if(empty($record->get_form_settings('helloplus_collect_entries_meta_data'))){
                return;
            }
        }else{
            if(empty($record->get_form_settings('collect_entries_meta_data'))){
                return;
            }
        }
